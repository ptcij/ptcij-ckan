...
write_config () {
  ...
  ckan-paster --plugin=ckan config-tool "$CONFIG" \
      ...
      # "licenses_group_url = file://${CKAN_HOME}/src/ckan/ckanext/ordbase-theme/ckanext/ordbasetheme/public/licenses.json"

  # other custom settings
  ckan-paster --plugin=ckan config-tool "$CONFIG" -f "/tmp/custom_settings.ini"
...
theme_setup () {
    cd "${CKAN_HOME}/src/ckan/ckanext/ordbase-theme" && python ./setup.py develop
}
